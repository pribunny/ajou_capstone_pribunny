...
const unfairDetectController = async (req, res) => {
  try {
    ...
    console.log('✅ 원본 detectText (HTML):\n', detectText);
    ...
    let markdownText;
    try {
      markdownText = htmlToMarkdown(detectText);
    ...
    const apiUrl = `http://backend-ml:8000/llm/unfairDetects/`;

    const modelResponse = await axios.post(apiUrl, requestBody, {
      headers: { 'Content-Type': 'application/json' }
    });

    // 모델 응답 테스트 전, 오류 발생 시 수정할 예정

    // 응답을 통합하는 함수
    function buildSummaryResponse(modelResponse) {
      if (!modelResponse.success) {
        throw new Error('탐지 실패: ' + (modelResponse.message || 'Unknown Error'));
      }
    ...
    console.error('unfairDetectController Error:', error);
    return res.status(500).json({
      success: false,
      code: 'SERVER_ERROR',
